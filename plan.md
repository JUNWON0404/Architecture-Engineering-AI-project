...
### 성능 & 확장성 개선 (Post-MVP)
- [ ] **데이터베이스**: 로컬/파일 기반 SQLite(`better-sqlite3` + Drizzle) → 호스팅된 PostgreSQL 등으로 이전 검토 (배포·동시 접속·백업 요구에 맞춤)
...
### 데이터 저장소 (현재)
- **SQLite** + **Drizzle ORM** (`DATABASE_URL`, `drizzle/` 마이그레이션). Google Sheets 연동 코드는 사용하지 않아 제거함.
